...
- Аргументы вычисляются на момент вычисления `defer`
```go
func a() {
    i := 0
    defer fmt.Println(i)
...
# Руны
Так как в го строка это массив байт, то символы unicode могут занимать больше чем байт и для них используются руны(rune).
...
# Gorutine
Легковесный поток в рантайме го. Имеет стек 2К.
Горутины управляются планировщиком го.
- G - горутина
- M - машина, поток ОС
