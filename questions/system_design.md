...
Партиционирование - разделение больших таблиц на несколько маленьких

Вертикальное партиционирование - разделение по колонкам на разные таблицы

Гаризонтальное партиционирование - разделение по строкам на разные таблицы

Шардирование - режем таблицы на кусочки, но шарды находятся на разных инстансах

#### Подходы к шардированию

- Range based - разбиваем по диапозонам
- Key based - вычисляем хеш и берем остаток от деления
- Directory based - статический список для шардов

#### Routing

- Client routing - приложение определяет шард
- Proxy routing - приложение не знает о шардинге
- Coordinator roting - как прокси но с доп.логикой, может в джойны, селекты, кэш и тд

#### Перебалансировка

- Запрещаем операции на запись и переливаем в другой шард
- Если данные не изменяются, можно писать на target, а читать из старого шарда
- Логическая репликация, после синхронизации переключается на новый шард
- Смешанный подход

#### Решардинг

Нужно добавить/удалить шард

- Hashing - остаток от деления, есть проблема со сменой шарда
- Consistent hashing - циферблат, при выбывании шарда ключ переедет далее по кругу. Может лавинообразно завалить всю базу
- Randevous hasing - вычисляется несколько хешей, ключ уезжает на шард с максимальным хешом
- Virtual buckets - создаем много вирутальных шардов, то есть каждому реальному шарду будет соответствовать N вируальных. При добавлении шардов они поменяются
...
Есть схема, есть отношения между таблицами, ACID, поддерживает язык SQL, нужно писать миграции для изменения схемы
